...
const SERVER_URL = "https://192.168.1.7:4000"; // same as your signaling server

async function fetchYTSearch(q) {
  const res = await fetch(
    `${SERVER_URL}/youtube/search?q=${encodeURIComponent(q)}`,
  );
  if (!res.ok) throw new Error("search failed");
  return res.json();
}

export default function YouTubeTogether({
  socketRef,
  matchedPeer,
  initialVideoId = "KflMqooMzF8",
}) {
  ...
  function selectVideo(videoId, broadcast = true) {
    ...
    if (broadcast) {
      const socket = socketRef?.current;
      if (socket && matchedPeer)
        socket.emit("yt-select", { to: matchedPeer, videoId });
    }
  }
          ...
          {results.map((it) => (
            ...
            >
              ...
              <div className="card-body p-3">
                <h2 className="card-title text-sm line-clamp-2">{it.title}</h2>
